 ...
 * - RGPD : Anonymise les réservations > 3 ans
 */
 ...
 */
/**
 * Coworking JSON Tools - VERSION SÉCURISÉE
 * Suppression de la clé secrète en dur - Utilisation des permissions WordPress
 */
...
/* ==============================================================
   ...
   - Utilise les permissions WordPress natives
   - Vérifie les nonces pour éviter les attaques CSRF
============================================================== */
...
add_action('rest_api_init', function() {
    ...
    register_rest_route('coworking/v1', '/rebuild-json/(?P<offre_id>\d+)', [
        ...
        'permission_callback' => function(WP_REST_Request $request) {
            // Seuls les utilisateurs avec les droits d'admin peuvent accéder
            return current_user_can('manage_options');
        },
        'callback' => function(WP_REST_Request $request) {
            // Vérifier le nonce pour la sécurité CSRF
    ...
    // C'est celui utilisé par le calendrier frontend
    register_rest_route('coworking/v1', '/availability/(?P<offre_id>\d+)', [
        'methods' => 'GET',
...
/* -------------------------------------------------------------
   4) AJAX endpoint pour l'admin (utilisé par le planning)
   - Plus sécurisé que REST pour les actions d'admin
   - Utilise les nonces WordPress
------------------------------------------------------------- */
add_action('wp_ajax_cw_rebuild_json_admin', function() {
    // Vérifier le nonce
...
/* -------------------------------------------------------------
   5) Mettre à jour le bouton dans le planning admin
   Pour utiliser l'AJAX plutôt que le formulaire direct
------------------------------------------------------------- */
function cw_update_planning_resync_button() {
    ?>
    <script>
    jQuery(document).ready(function($) {
        // Intercepter le formulaire de resynchronisation
        $('form[name="cw_resync_form"]').on('submit', function(e) {
            e.preventDefault();
            
            var form = $(this);
            var offreId = form.find('select[name="cw_resync_offre"]').val();
            var nonce = form.find('input[name="cw_resync_nonce"]').val();
            ...
            button.prop('disabled', true).text('Reconstruction en cours...');
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $('<div class="notice notice-success is-dismissible" style="margin-top:10px;">' +
                          '<p>✅ ' + response.data.message + ' (' + response.data.count + ' réservations)</p>' +
                          '</div>').insertAfter(form);
                        
                        // Actualiser le calendrier
                        $('#cw-cal-refresh').click();
                    } else {
...
add_action('admin_footer', 'cw_update_planning_resync_button');

/* -------------------------------------------------------------
   6) Remplacer l'ancien formulaire par la nouvelle version AJAX
   Dans cw_render_manual_blocks_manager() ou cw_render_maintenance_tools()
------------------------------------------------------------- */
function cw_render_maintenance_tools_secure() {
    $offres = get_posts([
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
        ...
        <!-- Outil 1 : Re-synchronisation JSON (version AJAX) -->
        <div style="background: #f8f9fa; padding: 15px; border-radius: 6px;">
            <h3 style="margin-top:0;">🔄 Re-synchroniser JSON</h3>
            <p style="font-size:13px; color:#666;">Force la reconstruction du JSON de disponibilité à partir des commandes WooCommerce.</p>
            
            <form name="cw_resync_form" method="post" action="">
                <?php wp_nonce_field('cw_resync_json', 'cw_resync_nonce'); ?>
                <select name="cw_resync_offre" required style="width:100%; margin-bottom:10px; padding:6px;">
...
/* -------------------------------------------------------------
   7) Test manuel sécurisé - Admin uniquement
------------------------------------------------------------- */
add_action('init', function() {
    // Ne permettre ce test qu'aux admins et en local/dev
    if (isset($_GET['test_rebuild_json']) && current_user_can('manage_options') && WP_DEBUG) {
 ...
 */
/**
 * Page Planning Coworking - Version Complète avec Corrections
 */
...
function cw_render_full_planning_page() {
    ...
    <div class="wrap">
        ...
        <!-- SECTION NOUVELLE : Réservation Admin -->
        <?php cw_render_admin_booking_form(); ?>
        ...
        <!-- SECTION 5 : Blocages manuels (AMÉLIORÉ) -->
        <div class="cw-section">
            <h2>🚫 Gérer les dates bloquées</h2>
            <?php cw_render_manual_blocks_manager(); ?>
        </div>
        
        <!-- SECTION 6 : Re-synchronisation JSON -->
        <div class="cw-section" style="border-left:4px solid #dc3545;">
            <h2 style="color:#dc3545;">🔄 Outils de maintenance</h2>
            <?php cw_render_maintenance_tools_secure(); ?>
        </div>
    </div>
    ...
    (function($) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.data.has_new) {
                        showNotification('Nouvelle réservation ! Rafraîchir la page pour voir.');
                    }
                }
            });
        }
        ...
        function showNotification(message) {
            ...
            const toast = $('<div class="notice notice-success is-dismissible" style="position:fixed; top:20px; right:20px; z-index:99999; min-width:300px;"><p>🔔 ' + message + '</p></div>');
            $('body').append(toast);
        ...
        // Initial check après 5 secondes
...
/* ============================================================
   NOUVELLE SECTION : Formulaire Réservation Admin
============================================================ */

function cw_render_admin_booking_form() {
    $offres = get_posts(['post_type'=>'offre-coworking', 'posts_per_page'=>-1]);
    ?>
    <div class="cw-section" style="border-left: 4px solid #10b981;">
        <h2>📞 Nouvelle Réservation (Admin / Téléphone)</h2>
        <p style="margin-top:0; color:#666; font-size:14px;">Utilisez ce formulaire pour créer une réservation suite à un appel téléphonique. Le système générera une commande WooCommerce.</p>
        
        <form method="post" action="" style="display:grid; grid-template-columns: 1fr 1fr 1fr auto; gap:15px; align-items:end;">
            <?php wp_nonce_field('cw_admin_book', 'cw_admin_book_nonce'); ?>
            
            <div>
                <label><strong>Client (Nom complet)</strong></label>
                <input type="text" name="adm_client_name" required style="width:100%" placeholder="Jean Dupont">
            </div>
            <div>
                <label><strong>Email (Optionnel)</strong></label>
                <input type="email" name="adm_client_email" style="width:100%" placeholder="client@email.com">
            </div>
             <div>
                <label><strong>Espace</strong></label>
                <select name="adm_offre_id" required style="width:100%">
            ...
            <div style="grid-column: 1 / -1; display:flex; gap:15px; margin-top:10px;">
                <div>
                    <label>Début</label>
                    <input type="date" name="adm_start" required min="<?php echo date('Y-m-d'); ?>">
                </div>
                <div>
                    <label>Fin</label>
                    <input type="date" name="adm_end" required min="<?php echo date('Y-m-d'); ?>">
                </div>
                <div>
                     <label>Formule</label>
                     <select name="adm_formule">
                 ...
                 <div style="margin-left:auto;">
                    ...
                    <select name="adm_payment_status">
                        <option value="pending">En attente de paiement (Envoyer lien)</option>
                        <option value="processing">Payé (Sur place / Virement)</option>
                    </select>
                </div>
                <div>
                    <button type="submit" name="submit_admin_booking" class="button button-primary button-large"> Créer Réservation</button>
                </div>
            </div>
        </form>
    </div>
    ...
    if (isset($_POST['submit_admin_booking']) && check_admin_referer('cw_admin_book', 'cw_admin_book_nonce')) {
        ...
        $status = sanitize_text_field($_POST['adm_payment_status']); // 'pending' ou 'processing'
        
        // Vérifier disponibilité
        if (function_exists('coworking_check_availability_with_locks')) {
            $check = coworking_check_availability_with_locks($offre_id, $start, $end);
            if (!$check['available']) {
                echo '<div class="notice notice-error is-dismissible"><p>❌ La période n\'est pas disponible : ' . date('d/m/Y', strtotime($check['fail_date'])) . '</p></div>';
                return;
            }
        }
        
        // 2. Création Commande WC (programmatically)
        $address = [
            'first_name' => $name,
            'email'      => $email,
            'phone'      => ''
        ...
        // Trouver le produit WC associé
        $product_id = cw_resolve_product_id_from_offre($offre_id); 
        if(!$product_id) $product_id = 1913; // Fallback Bureau
        ...
        // Ajouter les métadonnées critiques pour ton système
        $item->add_meta_data('_cw_offre_id', $offre_id);
        $item->add_meta_data('_cw_start', $start);
        $item->add_meta_data('_cw_end', $end);
        $item->add_meta_data('_cw_formule', $formule);
        $item->add_meta_data('_cw_price', $final_price);
        $item->save();

        $order->calculate_totals();
        $order->update_status($status, 'Réservation créée manuellement par l\'admin.');
        
        // Déclencher le traitement de finalisation
        if ($status === 'processing') {
            coworking_finalize_reservation($order->get_id());
        }

        echo '<div class="notice notice-success is-dismissible"><p>✅ Réservation créée ! Commande #'.$order->get_id().'</p></div>';
        
        // Astuce : Si "pending", afficher le lien de paiement pour que l'admin l'envoie par mail
        if($status === 'pending') {
            echo '<div class="notice notice-info"><p>🔗 Lien de paiement à envoyer au client : <strong>' . $order->get_checkout_payment_url() . '</strong></p></div>';
        }
    }
}

/* ============================================================
   SECTION 1 : Arrivées du jour (inchangée)
============================================================ */
...
/* ============================================================
   SECTION 5 : Blocages manuels (AMÉLIORÉ avec plage de dates)
============================================================ */
...
function cw_render_manual_blocks_manager() {
    ...
    echo '<form method="post" action="" style="background:#fff3cd;padding:20px;border-left:4px solid #ffc107; margin-bottom:25px;">';
    echo '<h3 style="margin-top:0;">🚫 Bloquer une période (Fermeture, Travaux...)</h3>';
    echo '<p style="margin-top:0; color:#666; font-size:14px;">Sélectionnez une plage de dates à bloquer. Le système générera automatiquement toutes les dates entre le début et la fin.</p>';
    ...
    // Choix Espace
    echo '<div><label style="font-weight:600;">Espace</label><br>';
    echo '<select name="cw_block_offre" required style="padding:6px;">';
    foreach ($offres as $offre) echo '<option value="' . $offre->ID . '">' . esc_html($offre->post_title) . '</option>';
    echo '</select></div>';

    // Date Début
    echo '<div><label style="font-weight:600;">Du</label><br>';
    echo '<input type="date" name="cw_block_start" required min="' . date('Y-m-d') . '" style="padding:6px;"></div>';

    // Date Fin
    echo '<div><label style="font-weight:600;">Au (inclus)</label><br>';
    echo '<input type="date" name="cw_block_end" required min="' . date('Y-m-d') . '" style="padding:6px;"></div>';

    // Raison
    echo '<div style="flex-grow:1;"><label style="font-weight:600;">Raison</label><br>';
    echo '<input type="text" name="cw_block_reason" placeholder="Ex: Maintenance" style="width:100%;padding:6px;"></div>';

    echo '<div><button type="submit" name="cw_add_block_submit" class="button button-primary">Bloquer</button></div>';

    echo '</div></form>';

    // --- TRAITEMENT PHP AMÉLIORÉ ---
    if (isset($_POST['cw_add_block_submit']) && check_admin_referer('cw_add_block', 'cw_block_nonce')) {
        ...
        $end   = sanitize_text_field($_POST['cw_block_end']); // Si vide, on prend start
        if(!$end) $end = $start;
        $reason = sanitize_text_field($_POST['cw_block_reason']);

        // Validation des dates
        if (strtotime($start) > strtotime($end)) {
            echo '<div class="notice notice-error is-dismissible"><p>❌ La date de début doit être avant la date de fin.</p></div>';
        } else {
            ...
            // Récupérer l'existant et fusionner
            ...
            echo '<div class="notice notice-success is-dismissible"><p>✅ Période bloquée avec succès (' . count($dates_to_block) . ' jours).</p></div>';
        }
    }
    ...
    jQuery(document).ready(function($) {
        $('.cw-unblock').on('click', function(e) {
            ...
            var element = $(this);
            var offre = element.data('offre');
            var date = element.data('date');
            
            if (!confirm('Débloquer la date ' + date + ' ?')) return;
...
// Handler AJAX pour débloquer une date
add_action('wp_ajax_cw_unblock_date', function() {
    ...
    $new_lines = array_filter($lines, function($line) use ($date_to_remove) {
        $parts = explode('#', $line);
        return trim($parts[0]) !== $date_to_remove;
    });
...
/* ============================================================
   SECTION 6 : Outils de maintenance
============================================================ */

function cw_render_maintenance_tools() {
    $offres = get_posts([
        'post_type' => 'offre-coworking',
        'posts_per_page' => -1,
        'post_status' => 'publish'
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
        ...
        <!-- Outil 1 : Re-synchronisation JSON -->
        <div style="background: #f8f9fa; padding: 15px; border-radius: 6px;">
            <h3 style="margin-top:0;">🔄 Re-synchroniser JSON</h3>
            <p style="font-size:13px; color:#666;">Force la reconstruction du JSON de disponibilité à partir des commandes WooCommerce. À utiliser si l'affichage semble incohérent.</p>
            ...
            <form method="post" action="">
                ...
                <button type="submit" name="cw_resync_submit" class="button button-secondary">Reconstruire le JSON</button>
            </form>
            ...
            if (isset($_POST['cw_resync_submit']) && check_admin_referer('cw_resync_json', 'cw_resync_nonce')) {
                ...
                if ($offre_id && function_exists('cw_rebuild_reservations_json')) {
                    $result = cw_rebuild_reservations_json($offre_id);
                    echo '<div class="notice notice-success" style="margin-top:10px;"><p>✅ JSON resynchronisé pour ' . get_the_title($offre_id) . '</p></div>';
                }
            }
            ?>
        </div>
        
        <!-- Outil 2 : Vérification d'intégrité -->
        <div style="background: #f8f9fa; padding: 15px; border-radius: 6px;">
            <h3 style="margin-top:0;">🔍 Vérifier intégrité</h3>
            <p style="font-size:13px; color:#666;">Compare les réservations CPT avec le JSON de disponibilité pour détecter les incohérences.</p>
            
            <form method="post" action="">
                <?php wp_nonce_field('cw_check_integrity', 'cw_check_nonce'); ?>
                <select name="cw_check_offre" required style="width:100%; margin-bottom:10px; padding:6px;">
                    <option value="">Choisir un espace...</option>
                    <?php foreach($offres as $offre): ?>
                        <option value="<?php echo $offre->ID; ?>"><?php echo esc_html($offre->post_title); ?></option>
                    <?php endforeach; ?>
                </select>
                <button type="submit" name="cw_check_submit" class="button button-secondary">Vérifier</button>
            </form>
            ...
            if (isset($_POST['cw_check_submit']) && check_admin_referer('cw_check_integrity', 'cw_check_nonce')) {
                ...
                if ($offre_id) {
                    cw_check_data_integrity($offre_id);
                }
            }
            ?>
        </div>
...
function cw_check_data_integrity($offre_id) {
    // Récupérer JSON
    $json = get_field('reservations_json', $offre_id) ?: '[]';
    $json_data = json_decode($json, true);
    $json_count = is_array($json_data) ? count($json_data) : 0;
    
    // Compter réservations CPT
    $cpt_args = [
        'post_type' => 'cw_reservation',
        'post_status' => 'publish',
        'posts_per_page' => -1,
    ...
    echo '<div class="notice ' . ($json_count === $cpt_count ? 'notice-success' : 'notice-warning') . '" style="margin-top:10px;">';
    echo '<p><strong>Résultats de vérification :</strong></p>';
    echo '<ul>';
    echo '<li>JSON de disponibilité : ' . $json_count . ' entrées</li>';
    echo '<li>Réservations CPT : ' . $cpt_count . ' entrées</li>';
    echo '</ul>';
    
    if ($json_count === $cpt_count) {
        echo '<p>✅ Les données sont synchronisées.</p>';
    } else {
        echo '<p>⚠️ Il y a une différence de ' . abs($json_count - $cpt_count) . ' entrée(s).</p>';
        echo '<p>Utilisez le bouton "Reconstruire le JSON" pour corriger.</p>';
    }
    echo '</div>';
...
/* ============================================================
   AJAX pour le Polling (Near Real-Time)
============================================================ */
...
/**
 * Coworking Admin Helpers
 */
/**
 * Coworking - Admin Helpers
 * Résolution des IDs WooCommerce pour la réservation Admin.
 */

// NOTE: Adaptez les IDs (1913 et 1917) si elles changent dans votre catalogue WooCommerce.
if (!function_exists('cw_resolve_product_id_from_offre')) {
    function cw_resolve_product_id_from_offre(int $offre_id) : int {
        // Logique basée sur le nom de l'offre (à adapter si vous avez un champ ACF de liaison)
        $title = get_the_title($offre_id);

        // Si le titre contient "réunion" (Salle de réunion)
        if (strpos(strtolower($title), 'réunion') !== false) {
            return 1917; // ID Produit WooCommerce de la Salle de réunion (Stock 1)
        }
        
        // Par défaut, Bureau privé
        return 1913; // ID Produit WooCommerce du Bureau privé (Stock 7)
    }
}
 ...
 */
/**
 * Coworking Notification System - Version Optimale
 */

/* ============================================================
   1. COMPTEUR DANS LE MENU ADMIN
============================================================ */
...
function cw_add_menu_notification_badge() {
    ...
    // On ne calcule que pour les admins
    ...
    // Ajouter le badge au menu Planning
...
/* ============================================================
   2. NOTIFICATION BARRE ADMIN AMÉLIORÉE
============================================================ */
...
/* ============================================================
   3. STYLE POUR LES NOTIFICATIONS
============================================================ */
